...
class App extends React.Component {
        ...
        deleteobject = (items) => {
            const newList = (this.state.ObjectData.length === 1) ? this.state.ObjectData.splice(items, -1) : this.state.ObjectData.splice(items, 1)
            const prepareOnly9Recod = [];
            this.state.data.forEach(item => {
            ...
            return ( <
                ...
                div className = "row" > {
                    data.map((item) => ( <
                            ...
                            img src = { item.url }
                            style = {
                                { width: '100%', height: '150px' } } > < /img>

                            <
                            p > {
                                (item.title.length <= 30) ? item.title : item.title.substring(0, 30) + ' ' + '...' } < /p> <
                            p > { item.id } < /p> <
                            p > { item.url } < /p> {
                                (item.flag === "true") ?
                                <
                        ...
                        ObjectData.length ?
                            ...
                            tr > < th colSpan = "4"
                        style = {
                                { textAlign: 'center', backgroundColor: "#000", color: "#fff" } } > COMPARISON TABLE < /th></tr >
                            <
                            tr >
                            <
                            th > Image < /th> <
                            ...
                            /tr> {
                                ObjectData.map((item) => ( <
                                    ...
                                    td > < img src = { item.url }
                                    style = {
                                        { width: '200px', height: '150px' } } > < /img></td >
                                    <
                                    td > { item.id } < /td> <
                                    td > { item.url } < /td> <
                                    td > { item.title } < /td> <
